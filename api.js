...
const API = {
    ...
    getGenres: async () => {
        return await fetchAPI(`/genre/movie/list?api_key=${API_KEY}`);
    },
    getMoviesByGenre: async (genreId) => {
        return await fetchAPI(`/discover/movie?api_key=${API_KEY}&with_genres=${genreId}&sort_by=popularity.desc`);
    }
};
